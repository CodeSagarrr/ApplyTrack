    ...
    try {
        ...
        if (file) {
            [result, parsedText, oldFile] = await Promise.all([
                uploadResume(file),
                parseResume(file.mimetype, file.buffer),
                Resume.findById({ _id: id, user: userId }).select("public_id"),
            ]);
        }
        ...
        const public_id = oldFile?.public_id;
        if (result) {
            try {
        ...
        const response = await Resume.findByIdAndUpdate({ _id: id, user: userId }, updateData, {
            new: true,
            runValidators: true,
        }).select("id fileName");
        if (!response) {
...
export const getResumes = async (req, res, next) => {
    try {
        const { userId } = req;
        const response = await Resume.find({ user: userId })
            .select("id user fileName versionName isDefault file_URL ats_score updatedAt createdAt")
            .lean();
        return res.status(200).json({
            success: true,
            data: response,
...
export const getSpecificResume = async (req, res, next) => {
    try {
        const { userId } = req;
        const { id } = req.params;
...
export const deleteResume = async (req, res, next) => {
    try {
        ...
        if (isExistInApplication && force === "true") {
            await Application.updateMany({ resume: id, user: userId }, { $set: { resume: null } }, { runValidators: true });
        }
...
export const updateResumeDefaultStatus = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { userId } = req;
        await Resume.updateMany({ user: userId }, { $set: { isDefault: false } });
        const response = await Resume.findByIdAndUpdate(id, {
            isDefault: true,
            updatedAt: new Date(),
