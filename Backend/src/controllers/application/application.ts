...
export const getAllApplications = async (
  ...
  try {
    ...
    const response = await Application.find({ user: userId as string })
      .select(
...
export const getFiltersApplication = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    if (status) {
      filters.status = status as IStatus;
    }
...
export const specificApplicationAtsService = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    if (application?.matchResult) {
      matchResultData = await MatchResult.findByIdAndUpdate(
...
export const atsService = async (
  ...
  try {
    ...
    const result = await runAnalysis(id as string, userId as string);
    return res.status(200).json({
      success: true,
      data: result,
    });
